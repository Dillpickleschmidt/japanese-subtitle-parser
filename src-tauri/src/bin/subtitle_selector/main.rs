...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if skipped_count > 0 {
        println!("Skipping {} episodes (already have better or equal quality)", skipped_count);
    }
    ...
    println!("Will download {} episodes:", filtered_selections.len());
    for (episode, candidate) in &filtered_selections {
        println!("  Episode {}: {} ({})", 
            episode, candidate.file_info.name, format_source_type(&candidate.source_type));
    }
        ...
        for (episode, current_choice) in current_choices {
            if !filtered_selections.contains_key(&episode) {
                ...
                let dummy_candidate = types::SubtitleCandidate {
                    ...
                    source_type: current_choice.source_type,
                    is_cc: false, // We don't track this
                    format: current_choice.format,
                    is_zip: current_choice.is_zip,
                    source_zip_url: None,
                };
