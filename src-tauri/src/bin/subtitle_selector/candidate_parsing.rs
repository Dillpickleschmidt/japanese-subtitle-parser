...
impl RegexPatterns {
    fn new() -> Self {
        Self {
            ...
            single_episode: Regex::new(r"(?:S\d+E|EO?|Episode\s*|Ep\s*|[^\d])(\d+)(?:[^\d]|$)")
                .unwrap(),
...
) -> Result<Vec<SubtitleCandidate>, Box<dyn std::error::Error>> {
...
fn filter_files(
    ...
    std::collections::HashMap<SourceType, JimakuFile>,
    Vec<JimakuFile>,
) {
...
fn parse_individual_candidate(
    file: JimakuFile,
    patterns: &RegexPatterns,
) -> Result<Option<SubtitleCandidate>, Box<dyn std::error::Error>> {
    let format = extract_format(&file.name);
    let episode_numbers = extract_episode_numbers(&file.name, patterns)?;
...
mod tests {
    ...
    fn test_extract_episode_numbers_single() {
        let patterns = RegexPatterns::new();
        assert_eq!(
            extract_episode_numbers("Inuyasha - 053 [Hi10].srt", &patterns).unwrap(),
            vec![53]
        );
        assert_eq!(
            extract_episode_numbers("S01E42.srt", &patterns).unwrap(),
            vec![42]
        );
        assert_eq!(
            extract_episode_numbers("Episode 7.srt", &patterns).unwrap(),
            vec![7]
        );
        assert_eq!(
            extract_episode_numbers("EO01.srt", &patterns).unwrap(),
            vec![1]
        );
    }
    ...
    fn test_extract_episode_numbers_range() {
        ...
        assert_eq!(
            extract_episode_numbers("Episodes 147-148.srt", &patterns).unwrap(),
            vec![147, 148]
        );
        assert_eq!(
            extract_episode_numbers("1-3.srt", &patterns).unwrap(),
            vec![1, 2, 3]
        );
        assert_eq!(
            extract_episode_numbers("001-167.zip", &patterns).unwrap(),
            (1..=167).collect::<Vec<_>>()
        );
        assert_eq!(
            extract_episode_numbers("[Group] Show EO01-167 (BD).zip", &patterns).unwrap(),
            (1..=167).collect::<Vec<_>>()
        );
        assert_eq!(
            extract_episode_numbers("[Group] Show E01-50.zip", &patterns).unwrap(),
            (1..=50).collect::<Vec<_>>()
        );
        assert_eq!(
            extract_episode_numbers(
                "[DmonHiro] Inuyasha E001-167 (BD, 720p) (US BD).zip",
                &patterns
            )
            .unwrap(),
            (1..=167).collect::<Vec<_>>()
        ...
        assert_eq!(
            extract_episode_numbers("Show 05-10.zip", &patterns).unwrap(),
            (5..=10).collect::<Vec<_>>()
        );
    }
    ...
    fn test_detect_source_type() {
        let patterns = RegexPatterns::new();
        assert_eq!(
            detect_source_type("Show S01E01 [BD].srt", &patterns),
            SourceType::BD
        );
        assert_eq!(
            detect_source_type("Show S01E01 [BluRay].srt", &patterns),
            SourceType::BD
        );
        assert_eq!(
            detect_source_type("Show S01E01 [retime].srt", &patterns),
            SourceType::FanRetime
        );
        assert_eq!(
            detect_source_type("Show S01E01 [DVD].srt", &patterns),
            SourceType::DVD
        );
        assert_eq!(
            detect_source_type("Show S01E01 [Netflix].srt", &patterns),
            SourceType::StreamDeprio
        );
        assert_eq!(
            detect_source_type("Show S01E01 [Hi10].srt", &patterns),
            SourceType::OtherWeb
        );
    }
