...
pub fn extract_words_from_transcripts(
    ...
    println!("Starting word extraction from {} transcripts", transcript_texts.len());
    ...
    for (i, chunk) in transcript_texts.chunks(BATCH_SIZE).enumerate() {
        println!("Processing batch {} of {} (transcripts {}-{})", 
                i + 1, 
        ...
        for ((word, reading, pos), transcript_ids) in chunk_result {
            word_map.entry((word, reading, pos)).or_default().extend(transcript_ids);
        }
    }

    println!("Word extraction completed. Found {} unique words", word_map.len());
...
fn fix_base_form_readings(
    ...
    let mut base_form_to_reading: HashMap<&str, String> = HashMap::with_capacity(unique_base_forms.len());
    ...
    for (batch_idx, batch) in unique_base_forms.chunks(BATCH_SIZE).enumerate() {
        println!("  Processing reading batch {} of {} ({} words)", 
                batch_idx + 1, 
    ...
    let mut corrected_word_map: HashMap<(String, String, Vec<String>), HashSet<i64>> = HashMap::with_capacity(word_map.len());
...
fn process_transcript_batch(
    ...
    println!("  Kagome processed {} transcripts in {:.2?}", batch.len(), start.elapsed());
        ...
        if let Some(tokens) = lines_of_tokens.get(line_idx) {
            for token in tokens {
                if is_content_word(token) {
                    word_map.entry((token.base_form.clone(), token.reading.clone(), token.pos.clone()))
                        .or_default()
